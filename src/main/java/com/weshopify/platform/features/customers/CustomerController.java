...
public class CustomerController {
	...
	public String customerAccessDeniedPage(Model model) {
		log.info("in access-denied method");

		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		String loggedInUser = authentication.getName();
		log.info("logged in user name is:\t" + loggedInUser);
		log.info("is Authenticated:\t" + authentication.isAuthenticated());
		log.info("authenticated user role is:\t");
		Collection<? extends GrantedAuthority> authzList = authentication.getAuthorities();
		authzList.forEach(ga -> {
	...
	@Transactional(isolation = Isolation.READ_COMMITTED, propagation = Propagation.REQUIRED, rollbackFor = Throwable.class, timeout = 3, readOnly = true)
	...
	public String customerSelfReg(@ModelAttribute("customer") @Valid CustomerBean customer,
			BindingResult validationResult, Model model) {
		List<String> errorList = new ArrayList<String>();
		log.info("is Customer Self Registered:\t" + customer.isSelfReg());
		log.info(customer.toString());
